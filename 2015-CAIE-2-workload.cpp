...
void Evala_WorkLoad(int seque[T_Oper][7],int FL[3])
{
	int i,j,z1,oc[Mac],MST[Mac][3],z3,z2[3],k,z,k2,k3,LS[Mac][T_Oper][8],flag,k1,z4,FT[NJb][3],tmp[7];
	int MachWorkLoad[Mac][3];
	for(i=1;i<=M;i++)
		...
		if(oc[seque[i][2]] == 0)
			{
				...
				if((Compare(a,b)==1)||(Compare(a,b)==-1)/*MST[seque[i][2]] >= FT[seque[i][0]]*/)
				{
					...
					MachWorkLoad[seque[i][2]][0]+=seque[i][3];
					MachWorkLoad[seque[i][2]][1]+=seque[i][5];
					MachWorkLoad[seque[i][2]][2]+=seque[i][6];
				}
				else
				{
		...
		else
			{
				...
				if(/*(j == oc[z1]) &&*/ (flag == 0))
				{
					...
					if((Compare(a,b)==1)||(Compare(a,b)==-1)/*MST[z1] >= FT[z3]*/)
					{
						...
						MachWorkLoad[z1][0]+=z2[0];
						MachWorkLoad[z1][1]+=z2[1];
						MachWorkLoad[z1][2]+=z2[2];
					}
					else
					{
						...
						MachWorkLoad[z1][1]+=z2[1];
						MachWorkLoad[z1][2]+=z2[2];
					//	if(	MachStarEnd[seque[i][2]][CC[seque[i][2]]][0]==0)
					//{
					//	system("pause");
					//}
					}
	...
	for(i=1;i<=M;i++)
	{
		...
		b[0]=MachWorkLoad[i][0];	 b[1]=MachWorkLoad[i][1];  b[2]=MachWorkLoad[i][2];
		if(Compare(a,b)==0)
		{
			FL[0] = MachWorkLoad[i][0];
			FL[1] = MachWorkLoad[i][1];
			FL[2] = MachWorkLoad[i][2];
		}
	}
